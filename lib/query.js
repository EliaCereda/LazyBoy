...
var Queriable = function () {
  ...
  this.all = function (options, cb) {
    ...
    if (options.shallow) {
      ...
      this._view(this.model_type + '/_shallow', {}, function (err, results) {
        if (err) {
          console.error('Query "' + self.model_type + '.all({ shallow: true }, ...)" failed with error "' + err.reason + '".');
          console.error('Did you set "enableShallowLoading: true" in the schema for ' + self.model_type + '?');
        }

        // FIXME: mark the objects as shallow to prevent saving them back to the db.

        return cb(err, results);
      });
    } else {
      this._view('LazyBoy/model_type', { key: this.model_type, include_docs: true }, cb);
    }
  };
