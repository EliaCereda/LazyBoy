...
Model.create_views = function (model_type, model_schema, model_options) {
  var model_views = {};
  ...
  if (model_options.enableShallowLoading) {
    var blacklisted = [];

    Object.keys(model_schema).forEach(function (property) {
      if (model_schema[property].type.has_one || model_schema[property].type.has_many) {
        blacklisted.push(property);
      }
    });
    ...
    model_views['lib'] = {
      'object-keys': 'module.exports = (' + polyfill.toString() + ')()'
    };

    model_views['_shallow'] = {
      map: 'function (doc) { ' +
        'if (!Object.keys) { ' +
          'Object.keys = require("views/lib/object-keys"); ' +
        '} ' +

        'if (doc.model_type === \''+model_type+'\') { ' +
          'var blacklisted = ' + JSON.stringify(blacklisted) + '; ' +
          'var copy = {}; ' +

          'Object.keys(doc).forEach(function (key, value) { ' +
...
Model.define = function (model_type, model_schema, model_options) {
  var schema  = create_schema(model_schema);
  var options = model_options || {};

  var model_document = {};
  model_document.schema = schema;
  model_document.views = Model.create_views(model_type, schema, options);
