...
Model.create_views = function (model_type, model_schema) {
  ...
  Object.keys(model_schema).forEach(function (property) {
    if (model_schema[property].indexed) {
      ...
      model_views[property].map = 'function (doc) { if (doc.model_type === \''+model_type+'\' && doc.' +property +') { emit(doc.' +property +', null)} }'
    }
  });
...
// Define global views that will be used for queries by all Models.

Model
  .define('LazyBoy', {})
  .addView('model_type', {
    map: 'function (doc) { emit(doc.model_type, null); }'
  });
